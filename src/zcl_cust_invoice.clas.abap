...
CLASS ZCL_CUST_INVOICE IMPLEMENTATION.
  ...
  METHOD get_pdf_64.
    ...
    CALL METHOD zadobe_ads_class=>getpdf
      EXPORTING
        template = 'ZCUSTOM_INVOICE/ZCUSTOM_INVOICE'     "<-write your template and schema name
        xmldata  = lv_xml
      RECEIVING
  ...
  METHOD build_xml.
    ...
    LOOP AT it_del_item INTO wa_del_item.
      ...
      IF wa_del_item-Batch IS NOT INITIAL.
        ...
        ELSE.
          lv_batch = |{ lv_batch }, { wa_del_item-Batch }|.
        ENDIF.
      ENDIF.
    ...
    "-------------------------------------------------
    " Format Manufacture Dates → DD-MM-YY
    "-------------------------------------------------
    ...
    "-------------------------------------------------
    " Gross weight → string (final output)
    "-------------------------------------------------
...
*AND plant = '1200'
    ...
    DATA: lv_comm_pct  TYPE p DECIMALS 2,
          lv_comm_kilo TYPE p DECIMALS 2,
           lv_comm_text1  TYPE string,
                    lv_comm_text  TYPE string.

    IF lv_comm1 IS NOT INITIAL.
      lv_comm_pct  =  lv_comm1 .
  lv_comm_text  = |{ lv_comm_pct }%|.
    ELSEIF lv_comm2 IS NOT INITIAL.
      lv_comm_pct  = lv_comm2.
        lv_comm_text  = |{ lv_comm2 }%|.
    ENDIF.


    IF lv_comm3 IS NOT INITIAL.
      lv_comm_pct  = lv_comm3 .
            lv_comm_text1  = |{ lv_comm3 } kg|.

    ELSEIF lv_comm4 IS NOT INITIAL.
      lv_comm_pct  = lv_comm4 .
    lv_comm_text1  = |{ lv_comm4 } kg|.
    ENDIF.

    DATA lv_comm_final TYPE string.
    IF lv_comm_text IS NOT INITIAL.
      lv_comm_final = lv_comm_text.
    ELSE.
      lv_comm_final = lv_comm_text1.
...
*    lv_amt_inword = num2words( iv_num = lv_amount_string ).
    ...
    |        <COUN_DES>{ wa_country_ship-countryname }</COUN_DES>| &&
    |        <remittance>{ wa_billdoc-YY1_RemittanceName_BDH }</remittance>| &&
    |      </SUBFORM8>| &&
    |    </SUUBFORM6>| &&
    |    <SUBFORM9>| &&
    |      <SUBFORM17>| &&
 ...
 |        </HeaderRow>|.
    ...
    |          <DS_GD>| &&
    |            <raw_mat>{ wa_sales_item-SalesOrderItemText }</raw_mat>| &&
    |            <hsn>{ lv_hsn }</hsn>| &&
    |            <gst>{ gst }</gst>| &&
    |            <igst>{ wa_lut_dlts-LutDescripton }</igst>| &&
    |            <enduser>{ endusercode }</enduser>| &&
    |            <licenc_des>{ wa_billdoc-YY1_LicenceDescription_BDH }</licenc_des>| &&
    |            <Pharmacopia>{ wa_sales_itemtp-YY1_Pharmacopiea1_SDI }</Pharmacopia>| &&
    |          </DS_GD>| &&
    ...
    |          <amt_mul>{ amt_mul }</amt_mul>| &&
    |        </ROW2>| &&
    |        <Row1>| &&
    |          <amt_usd>{ lv_total }</amt_usd>| &&
    |          <rs0>{ wa_saleshead-TransactionCurrency }</rs0>| &&
    |        </Row1>|.
...
*--------------------------------------
    ...
    |      </Table2>| &&
    |      <Subform4>| &&
    |        <description>{ lv_description }</description>| &&
    |        <amt_inwords>{ lv_amt_inword }</amt_inwords>| &&
    |        <COMM>{ lv_comm_final }</COMM>| &&
    |        <text_des1>{ wa_billdoc-YY1_AdvanceLicence_BDH }</text_des1>| &&
    |        <text_des2>{ wa_billdoc-YY1_DutyDrawback_BDH }</text_des2>| &&
    |        <text_des3>{ wa_billdoc-YY1_RoDTEP_BDH }</text_des3>| &&
    |      </Subform4>| &&
    |      <SUBFORM16>| &&
    |        <rs1>{ wa_saleshead-TransactionCurrency }</rs1>| &&
    |        <cif_val>{ wa_saleshead-IncotermsClassification }</cif_val>| &&
    |        <EX_RS>{ wa_billdoc-AccountingExchangeRate }</EX_RS>| &&
    |        <rs2>{ wa_saleshead-TransactionCurrency }</rs2>| &&
    |        <rs3>{ wa_saleshead-TransactionCurrency }</rs3>| &&
    |        <rs4>{ wa_saleshead-TransactionCurrency }</rs4>| &&
    |        <cip>{ lv_amount }</cip>| &&
    |        <freight>{ lv_freight }</freight>| &&
    |        <insurance>{ lv_insurance }</insurance>| &&
    |        <foc>{ lv_fob }</foc>| &&
    |        <CIP_V>{ amt_mul }</CIP_V>| &&
    |        <FR_RS>{ lv_rs_frg }</FR_RS>| &&
    |        <INS_RS>{ lv_rs_insure }</INS_RS>| &&
    |        <EXPT></EXPT>| &&
    |        <FOB_RS>{ lv_rs_fob }</FOB_RS>| &&
    |      </SUBFORM16>| &&
    |    </SUBFORM10>| &&
    |  </Subform2>| &&
    |  <BANK>{ wa_billdoctp-YY1_Bank_BDH }</BANK>| &&
    |</form1>|.
